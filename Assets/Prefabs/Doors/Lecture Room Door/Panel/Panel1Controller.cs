...
public class Panel1Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (currentAngle < angle + initialAngle - 1) transform.Rotate(Vector3.up, speed * Time.deltaTime); // 열기

        // 현재 각도가 목표하는 각도보다 클 때 (+ 1은 허용하는 오차 범위)
        else if (currentAngle > angle + initialAngle + 1) transform.Rotate(Vector3.down, speed * Time.deltaTime); // 닫기
