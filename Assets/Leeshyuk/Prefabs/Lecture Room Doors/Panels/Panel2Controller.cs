 ...
 *          
 *      2.  상위 프리팹 Lecture Room Door의 회전과 동기화되지 않음
 *
 */
using UnityEngine;
...
public class Panel2Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // 현재 각도
        float currentAngle = initialAngle - transform.eulerAngles[1];
        if (currentAngle < 0f) currentAngle += 360;

        // 현재 각도가 목표하는 각도보다 작을 때 (- 1은 허용하는 오차 범위)
        if (currentAngle < angle - 1) transform.Rotate(Vector3.down, speed * Time.deltaTime); // 열기

        // 현재 각도가 목표하는 각도보다 클 때 (+ 1은 허용하는 오차 범위)
        else if (currentAngle > angle + 1) transform.Rotate(Vector3.up, speed * Time.deltaTime); // 닫기

        // 각도가 범위를 벗어날 때, 오차로 인해 발생
        if (currentAngle < 0f || 90f < currentAngle) transform.rotation = Quaternion.Euler(0f, - angle + initialAngle, 0f); // 목표 각도로 바로 지정
    }
}